...
////// Emissive:
                ...
                float3 emissive = (_Basetexture_var.a*_ColorEmissive.rgb);
                float3 node_8183 = lerp(_Basetexture_var.rgb,_ShadowColor.rgb,(floor(pow(max(0,dot(normalDirection,lightDirection)),_Strength) * _ShadowEffects) / (_ShadowEffects - 1)*step(_ShadowSize,attenuation)));
                float3 finalColor = emissive + saturate(( node_8183 > 0.5 ? (1.0-(1.0-2.0*(node_8183-0.5))*(1.0-lerp(_Globalcolor.rgb,_ShadowColor.rgb,(step(_ShadowSize,attenuation)*(0.0*pow(max(0,dot(normalDirection,lightDirection)),_Strength)*_Thickness))))) : (2.0*node_8183*lerp(_Globalcolor.rgb,_ShadowColor.rgb,(step(_ShadowSize,attenuation)*(0.0*pow(max(0,dot(normalDirection,lightDirection)),_Strength)*_Thickness)))) ));
...
////// Lighting:
                ...
                float4 _Basetexture_var = tex2D(_Basetexture,TRANSFORM_TEX(i.uv0, _Basetexture));
                float3 node_8183 = lerp(_Basetexture_var.rgb,_ShadowColor.rgb,(floor(pow(max(0,dot(normalDirection,lightDirection)),_Strength) * _ShadowEffects) / (_ShadowEffects - 1)*step(_ShadowSize,attenuation)));
                float3 finalColor = saturate(( node_8183 > 0.5 ? (1.0-(1.0-2.0*(node_8183-0.5))*(1.0-lerp(_Globalcolor.rgb,_ShadowColor.rgb,(step(_ShadowSize,attenuation)*(0.0*pow(max(0,dot(normalDirection,lightDirection)),_Strength)*_Thickness))))) : (2.0*node_8183*lerp(_Globalcolor.rgb,_ShadowColor.rgb,(step(_ShadowSize,attenuation)*(0.0*pow(max(0,dot(normalDirection,lightDirection)),_Strength)*_Thickness)))) ));
